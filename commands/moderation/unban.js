...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(":lock:You Dont Have The Permissions To Unban Someone! - [BAN_MEMBERS]")

        if (!args[0]) return message.channel.send(":name_badge:Please Enter A Name!")
        ...
        if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send(":lock:I Don't Have Permissions To Unban Someone! - [BAN_MEMBERS]")
        try {
            if (reason) {
                ...
                var sembed = new MessageEmbed()
                    ...
                    .setDescription(`${bannedMember.user.tag} :unlock:has been unbanned for ${reason}`)
                ...
                var sembed2 = new MessageEmbed()
                    .setColor("GREEN")
                    .setAuthor(message.guild.name, message.guild.iconURL())
                    .setDescription(`${bannedMember.user.tag} :unlock:has been unbanned`)
