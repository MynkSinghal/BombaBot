  ...
  run: async (bot, message, args) => {
    if (!message.member.hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":lock:You Dont Have The Permissions To Unmute Someone!"
      );

    if (!message.guild.me.hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":lock:I Don't Have Permissions To Unmute Someone!"
      );
    if (!args[0])
      return message.channel.send(":name_badge:Please Enter A User!");
    let mutee =
    ...
    const sembed = new MessageEmbed()
      ...
      .setDescription(
        `${mutee.user.username} :unlock:was successfully unmuted.`
      );
