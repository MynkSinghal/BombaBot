...
module.exports = {
  ...
  usage:
    "[channel name | channel ID | channel mention] <role name | role ID | role mention]",
  ...
  run: async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "You Do Not Have Required Permissions! - [ADMINISTRATOR]!"
      );

    if (!args[0])
      return message.channel.send(
        "Please Enter A Channel Name Where The User Should Be Asked To Verify!"
      );

    if (!args[1])
      return message.channel.send(
        "Please Enter A Role Which Will Be Added After The User Is Verified!"
      );
    ...
    let channel =
      ...
      message.guild.channels.cache.find(
        c => c.name.toLowerCase() === args[0].toLocaleLowerCase()
      );
    ...
    try {
      ...
      if (channel.id === a && role.id === b) {
        return message.channel.send(
          "This Channel is Already Set As Verification Channel!"
        );
      } else if (channel.id === a && role.id === b) {
        return message.channel.send(
          "This Role is Already Set As Verification Role!"
        );
      } else {
        message.guild.channels.cache.forEach(channel => {
          if (channel.type === "category" && channel.id === verifiedchannel.id)
            return;
        ...
        return message.channel.send(
          `**Verification Channel And Role Has Been Set Successfully in \`${channel.name}\`!**`
        );
      }
    } catch {
      return message.channel.send(
