  ...
  run: async (bot, message, args) => {
    if (!message.member.hasPermission("MANAGE_ROLES"))
      return message.channel.send(
        ":lock:You Dont Have The Permissions To Add Roles To Users! - [MANAGE_ROLES]"
      );
    if (!message.guild.me.hasPermission("MANAGE_ROLES"))
      return message.channel.send(
        ":lock:I Dont Have The Permissions To Add Roles To Users! - [MANAGE_ROLES]"
      );
    ...
    if (message.guild.me.roles.highest.comparePositionTo(role) <= 0)
      return message.channel.send(
        ":lock:Role Is Currently Higher Than Me Therefore Cannot Add It To The User!"
      );
