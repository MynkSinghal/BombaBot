  ...
  run: async (bot, message, args) => {
    if (!message.member.hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":x:You Dont Have The Permissions To Warn Someone! - [MANAGE_GUILD]"
      );
    if (!args[0])
      return message.channel.send("Please Enter A <User> [reason]");
    ...
    if (reason) {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `**${target.displayName}Has Been Warned for ${reason}!**`
        );
      ...
      const embed = new MessageEmbed()
        ...
        .setAuthor(`${message.guild.name}`, message.guild.iconURL())
        .setDescription(
          `**${target.displayName}Has Been Warned!**`
        );
      message.channel.send(embed);
    }
