  ...
  run: async (bot, message, args) => {
    if (!message.member.hasPermission("MANAGE_ROLES"))
      return message.channel.send(
        ":lock:Your Dont Have The Permissions To Remove Role From Users! - [MANAGE_ROLES]"
      );

    if (!message.guild.me.hasPermission("MANAGE_ROLES"))
      return message.channel.send(
        ":lock:I Dont Have The Permissions To Remove Roles From Users! - [MANAGE_ROLES]"
      );

    if (!args[0])
      return message.channel.send(":id:Please Enter A Name Or Id User!");
    ...
    if (
      ...
      return message.channel.send(
        ":lock:Cannot Remove Role From This User! - [Higher Than Me In Role Hierachy]"
      );
    if (message.guild.me.roles.highest.comparePositionTo(role) < 0)
      return message.channel.send(
        ":lock:Role Is Currently Higher Than Me Therefore Cannot Remove It From The User!"
      );
    if (role.managed)
      return message.channel.send(
        ":lock:Cannot Remove That Role From This User!"
      );
